...
public class GeometryUtils {
    ...
    public static Path2D getPath2d(Connection connection){

        Path2D path = new Path2D.Double();
        List<Point2D> controlPoints = new ArrayList<>(connection.getHelperPoint());
        controlPoints.addFirst(connection.getSource().getPoint());
        controlPoints.addLast(connection.getTarget().getPoint());
        ...
        connection.setLength(GeometryUtils.calcPathLength(path));
        return path;
    }
